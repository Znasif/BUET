...
struct Triangle{
    Point point[3];
    double mnx[2]; // 0 holds smallest y
    mnx[0] = INFINITY; mnx[1] = -INFINITY;
    Point sect[3]; // sect[i] denotes point of intersection between edges with corners at i and i+1
    int seci[3];
    int color[3];
    ...
    void set_middle(){
        if(mnx[1] == min(mnx[1], Top_y)){
            int g = (mnx[1] - Top_y) / dy;
            mnx[1] = Top_y + g * dy;
        }
    }

    int get_intersection(double r){
        for(int i = 0; i<3; i++){
            int j = i + 1;
            if(j == 3) j = 0;
            sect[i].y = r;
            ...
            double t = (r - point[i].y)/(point[j].y - point[i].y);
            sect[i].x = point[i].x + t*(point[j].x - point[i].x);
            sect[i].z = point[i].z + t*(point[j].z - point[i].z);
            if(x_l_limit >= sect[i].x && sect[i].x <= -x_l_limit) seci[i] = 1;
        }
        for(int i = 0; i<3; i++){
            int j = i+1;
            if(j == 3) j = 0;
            if(seci[i]==1 && seci[j]==1){
                if(sect[i].x > sect[j].x){
                    Point temp;
                    temp = sect[i];
                    sect[i] = sect[j];
                    sect[j] = temp;
