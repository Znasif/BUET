...
struct Token{
	...
	int out;
	void Print2Matrix(){
		if(color=='G') {gFrame[15-now->y] |= 1<<[15-now->x];}
		else if(color=='R') {rFrame[15-now->y] |= 1<<[15-now->x];}
	}
	int getNum(){
		if(now==NULL) return -3;
		if(out==0) return -1;
		if(out==2) return -2;
		return now->num;
	}
...
void thisStep(Token* t, int dice){
	...
	if(t->out==1){
		...
		for (j=0;j<dice;j++)
		{
		    ...
		    if((t->now)->color==t->color && ((t->now)->next1!=NULL)){
				t->now=(t->now)->next1;
			}
			else{
				t->now=(t->now)->next;
...
int selectPlayer(int x, int y, int z){
	int g[4],i;
	Token* t;
	if(x==6){
		for(i=0;i<4;i++){
